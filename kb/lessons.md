...
## 2026-08-05 — Four wrong attributions in one debrief; one of them carried a strategic finding
**What:** Four attribution errors in the 8/5 debrief, all sourced from the Otter transcript, none caught before review. (1) **Invented a person** — Otter split a stretch of Charlie/Gloria Carter dialogue so a "Johnny" appeared to be a third party, reporting the injury and failing to print a sheet. Every one of those lines was Gloria. (2) **Wrong ME** — gave the powered-roller work to Ashwin; the receiving conveyors are copper, so it is Jad. Charlie names both MEs in safety conversations constantly, so transcript proximity says nothing about who owns a machine. (3) **Inverted an event** — presented Stephanie Brownlee as catching a repeat step-over on video 90 minutes after the stand-down. She caught nothing; she asked to record while people showed her *why* the step-over happens, and sent that demonstration to Kate. (4) **Manufactured a conflict** — read "I got the lockout tagout program now" against task #574 as a contradiction and built a political read on it. #574 is Jim performing LOTO assessments under the program.
**Why (3) is the entry:** it did not stay a name error. "Awareness control failed a live test in under two hours" became the lead hidden-risk finding, the argument for the 48-hour action, and a knowledge entry — all resting on an inference about what a video showed. Nothing in the transcript said a second violation occurred; it said a video existed and Kate reacted to it. The finding read as the strongest item in the debrief because it was the most specific.
**Lesson:** Otter does not only mangle names, it **invents speakers** by splitting one person's dialogue across an attribution boundary — an unfamiliar name in a two-person stretch is a transcription artifact until confirmed, not a person to create. And the ambiguity flag has to cover interpretation, not just ownership: "mentioned ≠ directed" governs who owns a task and says nothing about what an event meant, which is where the expensive error was. If a finding rests on an inferred event rather than a quoted one, say so on the line where it appears, and never let it carry a strategic conclusion unflagged. Specificity is not evidence.
