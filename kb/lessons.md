...
## 2026-08-05 — Four wrong attributions in one debrief, and a strategic finding built on top of one of them
**What:** The 8/5 debrief presented four attribution errors, all sourced from the Otter transcript and none caught before review. (1) **A person who does not exist.** Otter rendered a long stretch of Charlie-and-Gloria-Carter dialogue such that a "Johnny" appeared to be a third party — reporting the injury, failing to print the lessons-learned sheet. There is no Johnny. Every one of those lines was Gloria. (2) **Wrong ME.** Assigned the powered-roller work to Ashwin. The receiving conveyors are copper, so it is Jad. Charlie names both MEs in safety conversations often, so proximity in a transcript carries no signal about which one owns a machine. (3) **Inverted the meaning of an event.** Presented Stephanie Brownlee as having caught a repeat step-over on video ~90 minutes after the stand-down and escalated it to Kate. She caught nothing — she asked to record while people were showing her *why* the step-over happens, then sent the demonstration to Kate, where it read as a violation. (4) **Manufactured a conflict.** Read Charlie's "I got the lockout tagout program now" against open task #574 (*Assign Jim Melton to lockout/tagout program*) as a contradiction, and built a political read about quietly absorbing a program slated for the person who had applied for his job. #574 is Jim performing LOTO assessments under Charlie's program. No conflict existed.
**The one that mattered:** error (3) did not stay a name error. "Awareness control failed a live test in under two hours" became the lead hidden-risk finding, the argument for the 48-hour action, and a knowledge entry — all resting on an unverified inference about what a video showed. It read as the strongest item in the debrief precisely because it was the most specific. Nothing in the transcript actually said a second violation occurred; the transcript said a video existed and that Kate reacted to it.
**Fix:** Corrected records pushed the same session — tasks #759-771, knowledge #584-595 (with #585 written as the correction itself, so the demonstration-vs-violation distinction is in D1 rather than only in chat), intel #207-209, moves #49-50. Cameron duplicate #103 merged into #95. No Johnny record created.
**Lesson:** Otter does not only mangle names, it **invents speakers** by splitting one person's dialogue across an attribution boundary — so an unfamiliar name in a two-person stretch is a transcription artifact until confirmed, not a new person to create. Beyond names: a debrief inference about *what an event meant* deserves the same flag as an uncertain name. The existing discipline ("default ambiguous items to FYI, mentioned ≠ directed") covers task ownership but said nothing about interpretation, which is where the expensive error was. Rule going forward — if a finding rests on an inferred event rather than a quoted one, say so in the line where it appears, and never let it carry a strategic conclusion unflagged. Specificity is not evidence.
**Source:** 8/5 debrief review — Charlie corrections, 2026-08-05.

## 2026-08-02 — Cloudflare cron day-of-week is 1 = Sunday; every cron fired a day early for weeks
**What:** Investigating the "unexplained 07-27 backup miss" showed all three crons firing exactly one day before their intended day: five digest emails all Thursday 14:00 UTC (config said Friday), both auto-backup commits Sunday 12:00 UTC (config said Monday), the reflection auto-draft row written Thursday 21:00 UTC. Cloudflare's cron docs: day-of-week runs 1 = Sunday to 7 = Saturday (Quartz-style), unlike POSIX cron's 0 = Sunday. So `* * 5` meant Thursday and `* * 1` meant Sunday. Nothing ever failed — every run succeeded, on the wrong day. The "07-27 miss" was the backup running on schedule on Sunday 07-26. Consequence beyond confusion: the digest and the reflection draft summarized each week with Thursday and Friday missing, ~20% of every week since June.
**Fix:** v3.10.1 — day-of-week spelled out (`FRI`/`MON`) in wrangler.toml and in the `scheduled()` switch cases (`controller.cron` is the literal registered string, so both sides must change together). CLAUDE.md and deploy-notes corrected.
**Lesson:** Two. (1) Day-of-week numbers are ambiguous across cron dialects — always use three-letter names. (2) "Verify by outcome" needs the *right expected outcome*: the cadence check assumed Monday, so a healthy Sunday backup read as a Monday miss, and weeks of Thursday digests read as normal. When an artifact seems to miss its schedule, check what day the artifacts actually land before debugging the job. Also: the Cloudflare docs MCP tool answers platform-semantics questions from cloud sessions — no API token needed; don't declare root cause unreachable without checking it.
**Source:** Persistent-memory gap investigation, 2026-08-02.
