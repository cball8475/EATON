...
## 2026-08-06 — Debrief output was unreviewable: claims with no quotes, and the 05-12 over-attribution lesson repeated anyway
**What:** The scheduled sweep processed six meetings and presented ~38 items as prose bullets — each one my paraphrase, none carrying the transcript line behind it. Charlie's response was that he had no way to select what was accurate. He was right: verifying any single item meant re-listening to the meeting, so the "review before push" step was theatre. Worse, one item reproduced the **2026-05-12 over-attribution failure exactly** — an injury-case follow-up got filed as Charlie's when the transcript has him directing a coordinator to do it and the EHS manager saying she'd make the call herself. Otter's own action-item list had attributed it to `charlieball@eaton.com`, and I inherited that without checking it against the dialogue. Two summary-derived items also went in unflagged, one of them internally contradictory (a static pressure described as "meeting" a minimum it is below) because the insurance-audit transcripts were too large to fetch and I used Otter's summary without saying so.
**Fix:** `/debrief` rewritten. Every item now requires `ev` (verbatim quote, speaker label, Otter mistranscriptions left visible in brackets) and `src` (how the speaker was identified — labeled by Otter / inferred from context / no labels / summary-derived). Items are triaged by *action needed* rather than topic: tier 1 needs Charlie's call, tier 2 pushes unless he objects, tier 3 holds. Silence fails safe — untouched tier 2 pushes, untouched tiers 1 and 3 do not. Output is a tap-through Artifact with localStorage persistence and a parseable decision block, not prose.
**Lesson:** Three. (1) **A review step without evidence is not a review step.** If checking a claim costs more than accepting it, every claim gets accepted — the approval gate silently becomes a rubber stamp, which is the same class of failure as a cron that reports success. (2) **Otter's `action_items` list is a claim about ownership, not a source for it.** It attributes to the account holder by default; ownership must be read off the dialogue every time. This is why the 05-12 lesson didn't hold — it said "default ambiguous to FYI" but never named where the bad attribution actually enters. (3) An unfetchable transcript is not a smaller transcript. Summary-derived items are a paraphrase of a paraphrase and must be labelled as such at the item level, or they read exactly like quoted ones.
**Source:** Charlie correction during the 08-06 scheduled debrief sweep.
