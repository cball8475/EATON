...
## 2026-08-06 — A jq quoting bug made two "append" PATCHes overwrite the entry instead
**What:** Appending debrief updates to knowledge #587 and #455. The pattern was read-current → concatenate → PATCH. The read used `jq -r '(.knowledge[]?//.[]?)|select(.id==587)|.detail'` — no spaces around `//`, so jq parsed `?//` as the alternative-destructuring operator and failed to compile. The command substitution returned empty, the concatenation produced only the new paragraph, and the PATCH wrote that as the whole `detail`. Both entries lost their original text. **The jq error printed to stderr but the PATCH still returned `{"id":587,"err":null}`** — a clean success on a destructive write. Caught within the minute by a length check (587 was 1025 chars of history, came back 490 of new text) and restored from the read taken moments earlier; had that read not still been in context the content was gone, since D1 keeps no row history outside the Monday backup.
**Fix:** Both entries restored to original + append (587 → 1025 chars, 455 → 1284). Stopped interpolating multi-line content through shell variables — write the full intended value to a file and build the JSON with `jq -Rs '{detail:.}' file`, which cannot silently produce an empty string. Verified every subsequent PATCH with a length-and-head check rather than trusting the response.
**Lesson:** Three. (1) `//` in jq needs surrounding spaces or it collides with `?//`; the same paths worked earlier in the session *with* spaces, so this was a transcription slip, not a knowledge gap. (2) A read-modify-write against the system of record must fail closed — if the read comes back empty, abort; never let empty concatenate into a full-field overwrite. A PATCH whose new value is shorter than the old is a red flag worth asserting on. (3) `err:null` in the API response means the write succeeded, not that it was correct — same class as the crons that reported success while doing the wrong thing. Verify by reading the row back, not by reading the response.
**Source:** /debrief push, Press Maintenance and Safety Briefing 2026-08-06.

## 2026-08-06 — Otter transcribed Gireesh Challawar as "Joey" for a whole meeting
**What:** The 8/6 press briefing had Charlie saying he would ask "Joey" at Tier 3 for production coverage. No Joey exists in the 195-row people table. Charlie corrected it to Gireesh Challawar (id 55, Fab Ops Manager). Not a near-miss spelling like Jad→John or Sean→Shawn — a completely unrelated name, so a variant search would never have found it.
**Lesson:** Add "Joey" to the known Otter name-error list. More usefully: when a debrief produces an action item aimed at someone with **no** D1 person record, treat the name as suspect before treating it as a new hire. The correction mattered beyond spelling — Gireesh shares the forklift goal with Charlie and is the person Kate routes supervisor non-compliance to, so the ask sits differently than a favour from an unknown manager, and the PPE finding from the same walk is in his area.
**Source:** /debrief review, Press Maintenance and Safety Briefing 2026-08-06.
