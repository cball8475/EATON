...
- **Tables:** tasks, people, templates, knowledge, people_intel, leadership_moves, weekly_reflections, scoreboard
...
### Deploy Patterns
**Cloudflare Worker:** curl PUT to Cloudflare API with multipart form data. Secrets (API_TOKEN, ANTHROPIC_API_KEY) must be re-set after PUT-based redeployments.
...
- **D1** (via Worker API) — authoritative for everything dynamic: tasks, people, knowledge, intel, leadership_moves, weekly_reflections. Always pull fresh; don't cache facts here.
- **CLAUDE.md** (this file) — authoritative for stable rules: communication style, infra IDs, key people roster, priorities, targets. Auto-loaded every session.
- **ref/eaton-stable.md** — facility procedures (PPE, MOC, fab flows, EHS systems, shift supervisors, environmental). Read on demand for operational questions.
- **ref/debrief-protocol.md** — user-facing copy-paste prompt. Charlie's tool, not Claude's reference.
- **kb/lessons.md** — active failure log. The only `.md` file ever appended to. Force-read at `/morning` if updated since last session.
...
## File Organization
```
ref/eaton-stable.md       — Facility procedures, PPE, MOC, fab flows, EHS systems
ref/debrief-protocol.md   — User-facing copy-paste prompt for /debrief
ref/archive/              — Retired duplicates (do not read)
infra/worker-api.mjs      — Live Worker source (deploy from here)
infra/deploy-notes.md     — Deploy patterns + gotchas
tools/                    — Floor form (html + jsx) + 1-on-1 coach jsx
kb/lessons.md             — Active failure log (only .md file appended to)
kb/archive/               — Pre-D1-migration snapshots (do not read)
.claude/commands/         — Slash command definitions
.claude/settings.local.json — Permissions + MCP config
```
